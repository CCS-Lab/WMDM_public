...
parameters {
  ...
  vector[N] pi_wm_pr;      // Pavlovian bias under WM load
  vector[N] ep_wm_pr;      // learning rate under WM load
}
...
model {
  ...
  pi_wm_pr  ~ normal(0, 1.0);
  ep_wm_pr  ~ normal(0, 1.0);
  ...
  for (i in 1:N) {
    for (bl in 1:Bsubj[i]){
      ...
      real pi_tmp;     // temporary variable for storing pavlovian bias on a trial
      real ep_tmp;     // temporary variable for storing learning rate on a trial
      ...
      for (t in 1:Tsubj[i, bl]) {
        ...
        // Make a choice
        wv_g[cue[i, bl, t]]  = qv_g[cue[i, bl, t]] + b[i] + pi_tmp * sv[cue[i, bl, t]];
        ...
        {  // noise
          pGo[cue[i, bl, t]]   *= (1 - xi[i]);
          pGo[cue[i, bl, t]]   += xi[i]/2;
        }
        pressed[i, bl, t] ~ bernoulli(pGo[cue[i, bl, t]]);
        
        // after receiving feedback, update sv[t + 1]
        if (outcome[i, bl, t] >= 0) {
          sv[cue[i, bl, t]] += ep_tmp * (rhoRew[i] * outcome[i, bl, t] - sv[cue[i, bl, t]]);
        } else {
          sv[cue[i, bl, t]] += ep_tmp * (rhoPun[i] * outcome[i, bl, t] - sv[cue[i, bl, t]]);
        }
  
        // update action values
        if (pressed[i, bl, t]) { // update go value
          if (outcome[i, bl, t] >=0) {
            qv_g[cue[i, bl, t]] += ep_tmp * (rhoRew[i] * outcome[i, bl, t] - qv_g[cue[i, bl, t]]);
          } else {
            qv_g[cue[i, bl, t]] += ep_tmp * (rhoPun[i] * outcome[i, bl, t] - qv_g[cue[i, bl, t]]);
          }
        } else { // update no-go value
          if (outcome[i, bl, t] >=0) {
            qv_ng[cue[i, bl, t]] += ep_tmp * (rhoRew[i] * outcome[i, bl, t] - qv_ng[cue[i, bl, t]]);
          } else {
            qv_ng[cue[i, bl, t]] += ep_tmp * (rhoPun[i] * outcome[i, bl, t] - qv_ng[cue[i, bl, t]]);
          }
        }
      } // end of t loop
    }
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      for (bl in 1:Bsubj[i]){
        ...
        vector[4] sv;    // stimulus value
        vector[4] pGo;   // prob of go (press)
        real pi_tmp;     // temporary variable for storing irreducible noise on a trial        
        real ep_tmp;     // temporary variable for storing learning rate on a trial
