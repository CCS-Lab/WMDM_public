...
allConds = {'DV_wmgng_vs_gng'};  % all contrasts
...
for i = 1:length(allConds)
    ...
    %% specification
    matlabbatch = [];
    mkdir( fullfile(FOLDER_OUTPUT, dir_name ))
    
    scanFiles = [];
    c = 0;
    for j = 1:length(subjIDs_name)
        SUBJ = subjIDs_name{j};
        if exist( fullfile(FOLDER_1st, SUBJ ,'/ses-1/func', ['/con_', contrast_num, '.nii'] ))
            c = c + 1;
            tmp_scanFile1 = fullfile(FOLDER_1st, SUBJ, '/ses-1/func', ['/con_', contrast_num, '.nii,1']);  % if using subjIDs_name
            scanFiles{c,1} = tmp_scanFile1;
        end

    end

    matlabbatch{1}.spm.stats.factorial_design.dir = { fullfile(FOLDER_OUTPUT, dir_name ) };
    ...
    if exist(fullfile(FOLDER_OUTPUT, dir_name, 'SPM.mat'))
        fprintf('\n SPM.mat exists in this directory. Overwriting SPM.mat file! \n\n')
        delete(fullfile(FOLDER_OUTPUT, dir_name, 'SPM.mat'))
    end
    ...
    %% estimation
    matlabbatch = [];
    matlabbatch{1}.spm.stats.fmri_est.spmmat = { fullfile(FOLDER_OUTPUT, dir_name, 'SPM.mat') };
    matlabbatch{1}.spm.stats.fmri_est.method.Classical = 1;
    spm_jobman('run', matlabbatch)
    
    %% T-contrast (one-step t-test)
    matlabbatch = [];
    matlabbatch{1}.spm.stats.con.spmmat = { fullfile(FOLDER_OUTPUT, dir_name, 'SPM.mat') };
